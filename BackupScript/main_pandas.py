    ...
    for index, row in df_all.iterrows():
        ...
        for flag in flag_house:
            if (flag.lower() in row["Description"].lower()) & (skip == False):
                ...
                skip = True
                if "ferraresi" in row["Description"].lower(): #If rent was paid when I was in Africa rename it to daniel
                    df_all.loc[index, "Description"] = "Daniel Dougas"
                if ("ferraresi" in row["Description"].lower())&(row["month"] == 11): #remove the deposit paid back
                    df_all.loc[index, "month"] = 7
                if ("daniel pokras" in row["Description"].lower())&(row["year"] == 2025): # Change month deposit house
                    df_all.loc[index, "month"] = 1
        #Taxes
        for flag in flag_taxes:
            if (flag.lower() in row["Description"].lower()) & (skip == False):
                df_all.loc[index, "category"] = "taxes"
                if pd.notna(row["Subject"]):
                    if ("withdrawal" in row["Subject"].lower()): #Withdrawing from IB comes only with my name, same as taxes
                        df_all.loc[index, "category"] = "investment"
                        df_all.loc[index, "Description"] = "interactive brockers llc"
                skip = True
        ...
        for flag in flag_transportation:
            if (flag.lower() in row["Description"].lower()) & (skip == False):
                ...
                skip = True
                if ("SBB" in row["Description"])&(row["Amount"]<=-700): #halb tax plus
                    print(f"halb tax plus: {row['Amount']}")
                elif ("SBB" in row["Description"])&(row["Amount"]<=-500)&(row["Amount"]>=-700): #Exception for taking dollars bike trip
                    df_all.loc[index, "category"] = "holidays"
                    df_all.loc[index, "Description"] = "SBB_dollars"
                elif ("SBB" in row["Description"])&(row["Amount"]<=-64): #Exception for snow n rail
                    if (row["month"] <= 3) | (row["month"] >=11):
                        df_all.loc[index, "category"] = "sport"
                        df_all.loc[index, "Description"] = "Ski pass"
        #Insurance
        ...
        for flag in flag_other:
            if (flag.lower() in row["Description"].lower()) & (skip == False):
                skip = True
                if (flag == "Galaxus") & (row["Date"]>datetime(2023,4,1)) & (row["Date"]<datetime(2023,7,15)):# exception for the bike trip
                    df_all.loc[index, "category"] = "holidays"
                elif flag == "Credit Suisse (Schweiz) AG": #This is acutally swisscards
                    df_all.loc[index, "Description"] = "Swisscard AECS"
                else:
            ...
            if (flag.lower() in row["Description"].lower()) & (skip == False):
                ...
                for flag_t in flag_twint:
                    if (flag_t.lower() in df_all.loc[index, "Description"].lower()) & (abs(df_all.loc[index, "Amount"]) <= 110)&(skip==False):  #Twint must be below 200 chf otherwise holidays
                        ...
                        df_all.loc[index, "category"] = "holidays"    
                        df_all.loc[index, "Description"] = "Twint"
                        if ((row["Amount"]) < 550) & (row["Amount"]> 500) & (row["year"] == 2024) & (row["month"] == 9): #IT bike trip
                            df_all.loc[index, "month"] = 8
    
                if "Uber" in df_all.loc[index, "Description"]:
                    df_all.loc[index, "Description"] = "Uber eat"
                skip = True
        if ("Zahlverbindung Postfinance".lower() in df_all.loc[index, "Description"].lower()) & (df_all.loc[index,"year"] <= 2022): #30 bday party
            df_all.loc[index, "Description"] = "Twint"
            df_all.loc[index, "category"] = "restaurant"
        
        if ("ZKB" in df_all.loc[index, "Description"]): #30 bday party
            df_all.loc[index, "Description"] = "Cash withdraw"
        #Holidays
        if ((row["Amount"])>809) & (row["Amount"]<810) & (row["year"] == 2024) & (row["month"] == 4): #Colombia trip
            df_all.loc[index, "month"] = 3
        for flag in flag_holidays:
                if (flag.lower() in row["Description"].lower()) & (skip ==False):
                    skip = True
                    df_all.loc[index, "category"] = "holidays"

                    if (df_all.loc[index, "Amount"] < -770) & (df_all.loc[index, "Amount"]>-780):
                        pass
                    if ("yasmine" in row["Description"].lower()) | ("41789016324" in str(row["Subject"])):
                        df_all.loc[index, "Description"] = "yassou"
                        if row["year"] == 2023: #Namibia trip
                            df_all.loc[index, "month"] = 7
                        elif (row["year"] >= 2025) & (row["Amount"] >1100) & (row["Amount"] < 1300):
                                df_all.loc[index, "category"] = "housing"
                        elif (row["year"] == 2025) & (row["month"] == 3): #Moving in togther and deposit
                            df_all.loc[index, "month"] = 1
                            df_all.loc[index, "category"] = "housing"

                    elif ("fernandez" in row["Description"].lower())&(row["year"] == 2023)&(row["month"] == 11):
                        df_all.loc[index, "month"] = 10
                        df_all.loc[index, "Description"] = "SAFARI CAR RENTAL CC"
                    elif ("bucher" in row["Description"].lower())&(row["year"] == 2024)&(row["month"] == 1):
                        df_all.loc[index, "month"] = 10
                        df_all.loc[index, "year"] = 2023
                        df_all.loc[index, "Description"] = "SAFARI CAR RENTAL CC"
        #Entertainment
